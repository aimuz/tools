...
export function stripLocaleFromPath(pathname: string): string {
  ...
  if ((LOCALES as readonly string[]).includes(head) && head !== DEFAULT_LOCALE) {
    return rest.join('/');
  }
  return cleaned;
}

export function absoluteUrl(locale: Locale, pathname: string, origin: string = SITE_ORIGIN): string {
  return `${origin.replace(/\/$/, '')}${localizedPath(locale, pathname)}`;
}

export type HreflangEntry = { hreflang: string; href: string };

export function buildHreflangEntries(pathname: string, origin: string = SITE_ORIGIN): HreflangEntry[] {
