...
- **占位符**（**严禁翻译**）：任何 `*Template` / 含 `{foo}` 的字符串里的 `{key}` 是运行时字面量替换的变量名，6 份 locale 必须完全一致。反面教材：`'File too large. Max 5MB, current {size}'` → `'Fichier trop volumineux. 5 Mo maximum, {taille} actuelle'` ❌（`{taille}` 永远匹配不到，页面会渲染出字面量花括号）。正确做法：只翻译描述文案，`{size}` / `{count}` / `{label}` / `{fromLabel}` / `{pct}` / `{msg}` 等 key 原样保留——`'Fichier trop volumineux. Max 5 Mo, actuellement {size}'` ✅。查一眼 `ja.ts` / `ko.ts` 现有条目就知道所有 locale 的占位符拼写都一样
...
11. **i18n 模板占位符（`{size}` / `{count}` / `{label}` 等）在所有 locale 必须拼写完全一致**——运行时按 key 做字面量 `replace`，把 `{size}` 翻译成 `{taille}` / `{サイズ}` / `{크기}` 会让替换失败、花括号字面量直接渲染到用户界面。只翻译占位符**周围的描述文案**，key 保持原样。详见"本地化原则"章节的"占位符"条目
